...
# Adjustment factor to match Bouchet choice
...
if not pathlib.Path('restart.h5').exists():
    q = solver.state['q']
    #ff = forcing(1.0)
    q['c'] = forcing(1.0)*np.sqrt(2.0*Friction) + np.logical_and(nxg_local==3, nyg_local==0)*0.3
...
# This is the actual
snapshots = solver.evaluator.add_file_handler('snapshots', sim_dt=snapshotStep, max_writes=10, mode=fh_mode)
...
try:
    ...
    while solver.proceed:
        ...
        if (solver.iteration-2) % output_cadence == 0:
            ...
            logger.info('Max energy density = %f' % flow.max('Energy'))
            logger.info('Max enstrophy density = %f' % flow.max('Enstrophy'))
            curr_time = next_time
